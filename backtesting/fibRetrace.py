...
def check_exit(bar, symbol, time, portfolio, trade_log, portfolio_value):
    if symbol in portfolio and portfolio[symbol]["status"] == "open":
        ...
        half_qty = portfolio[symbol]["half_qty"]
        ...
        # Take profit at 0% level (sell half)
        if not portfolio[symbol]["half_exited"] and bar.close >= target:
            exit_price = round(bar.close, 2)
            pnl = round((exit_price - entry) * half_qty, 2)
            portfolio_value += pnl
            trade_log.append([
                symbol,
            ...
            portfolio[symbol]["half_exited"] = True
            portfolio[symbol]["quantity"] = half_qty

        # Stop out remaining half
        if bar.close <= portfolio[symbol]["sl"]:
            exit_price = round(bar.close, 2)
            pnl = round((exit_price - entry) * portfolio[symbol]["quantity"], 2)
            portfolio_value += pnl
            trade_log.append([
                symbol,
                portfolio[symbol]["time"].astimezone(mountain),
