...
def export_results(all_trade_logs, custom_filename=None):
    ...
    with open(filename, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Symbol", "Entry Time (MT)", "Entry Signal", "Entry Price", "Stop Loss", "Target", "Exit Price", "Exit Type", "P/L", "Exit Time (MT)", "R/R Ratio"])
        ...
        for bucket, values in rr_buckets.items():
            ...
            writer.writerow([f"{bucket} R/R", f"Avg P/L: {avg_bucket}", f"Total P/L: {total_bucket}", f"Count: {len(values)}"])
